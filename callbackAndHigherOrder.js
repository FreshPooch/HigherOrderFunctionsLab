...
// CODE HERE
const printArray = uniqArr => `The new names array with all the duplicate items removed is ${uniqArr}.`
console.log(unique(names, printArray)); 
...
// CODE HERE 
const each = (arr, cb) => {
//  return arr.forEach(name => cb(arr[name], ) ); 
for (let i = 0; i<arr.length; i++) {
 return cb(arr[i], i); 
}
}
...
const string = (item, index) => `The item at index ${item} is ${index}.`
console.log(each(names,string))
////////// PROBLEM 7 //////////
...
const addNUMBER = addingFactory(6); 
